...
class Dataset(object):
    ...
    def __init__(self, mix_scp="", ref_scp=None, sample_rate=8000):
        self.mix = WaveReader(mix_scp, sample_rate=sample_rate, normalize=True)
        self.ref = [
            WaveReader(ref, sample_rate=sample_rate, normalize=True) for ref in ref_scp
        ]
...
class ChunkSplitter(object):
    ...
    """
    def __init__(self, chunk_size, train=True, least=16000, make_stft=True):
        self.chunk_size = chunk_size
        self.least = least
        self.train = train
        self.make_stft = make_stft
    ...
    def _make_chunk(self, eg, s):
        ...
        #if self.make_stft:
            #chunk["mix"] = np.abs(librosa.stft(chunk["mix"], n_fft=1024, hop_length=160, win_length=640))
            #chunk["ref"] = [np.abs(librosa.stft(x, n_fft=1024, hop_length=160, win_length=640)) for x in chunk["ref"]]
        return chunk
    ...
    def split(self, eg):
        ...
        if N < self.chunk_size:
            ...
            chunk = dict()
            if self.make_stft:
                chunk["mix"] = np.append(eg["mix"], eg["mix"][0:P])
                chunk["ref"] = [np.append(ref, ref[0:P]) for ref in eg["ref"]]
                #chunk["mix"] = librosa.stft(chunk["mix"], n_fft=1024, hop_length=160, win_length=640)
                #chunk["ref"] = [librosa.stft(x, n_fft=1024, hop_length=160, win_length=640) for x in chunk["ref"]]
            else:
